...
- Migration 015 adds a per-recipient inbox with unique event/recipient identity, persistent read state, allowlisted generic messages/destinations, bounded cursor pagination, category/in-app preferences and read-time chat membership/block checks. The worker records in-app history before external gateways. Member-only HTTP APIs, web and Flutter inboxes passed the cited CI. Migration 016 adds governed follows and mark-unread, subject to a new CI run; do not count these as accepted until then.
