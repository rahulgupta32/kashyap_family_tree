...
- Draft completion PR #5: `feat/application-completion` → `feat/m4-governed-workflows`. Commit `c29c4f8f1947fad22fc7538d5b4afdbffe9aba9b` preserves all 392 repository files, including every M4 path. Chat, household consent, dashboard, audit and persistent inbox work is published.
...
- Current unpublished work adds user-selected person, immediate-family, relationship-group, branch and generation follows, gated by current visibility and branch authority. Approved genealogy edits fan out to authorized followers, while unfollow stops future delivery. Mark-unread is available in web and mobile inboxes.
...
- Completion `c29c4f8f1947fad22fc7538d5b4afdbffe9aba9b` contains 392 paths. [PR run 36032716021](https://github.com/rahulgupta32/kashyap_family_tree/actions/runs/36032716021) passed all four jobs: 110 backend unit, 195 PostgreSQL integration, 17 browser scenarios, Flutter analysis and widgets, live ClamAV and live API/PostgreSQL Android emulator. The corrected keyboard-dismissal and hit-test helper was exercised on Android.
- Migration 015 adds a per-recipient inbox with unique event/recipient identity, persistent read state, allowlisted generic messages/destinations, bounded cursor pagination, category/in-app preferences and read-time chat membership/block checks. The worker records in-app history before external gateways. Authenticated HTTP APIs, web and Flutter inboxes passed the cited CI. Migration 016 adds governed follows and mark-unread; commit `1d314f4f97b9ed88d3bdafa9355f444f287117c4` passed 110 unit, 196 real PostgreSQL integration, 18 browser, Flutter analysis/widgets and live ClamAV in [PR CI 36034989513](https://github.com/rahulgupta32/kashyap_family_tree/actions/runs/36034989513). Android is still running. A mobile notification navigation correction and mocked widget checks remain local until published and verified.
