...
**Base Branch**: `develop`  
**Verified Foundation Commit**: `b786663806a72e811c05d762f0f8a84617be3450`  
**Date**: 2026-09-11  
...
* **Automated Script**: [`scripts/ensure-kashyap-pg.sh`](../../scripts/ensure-kashyap-pg.sh) has been hardened with:
  ...
  - **Mandatory Pre-Cluster-Start Configured Data Directory Check**: Before cluster start, runs `pg_conftool` without error suppression (no `|| true`). Strictly requires non-empty, successfully parsed, and canonicalized path resolving inside `/mnt/kashyap_pg`. Fails fast with nonzero exit on any error.
  - **Mandatory Post-Cluster-Start Running Data Directory Check**: Queries postmaster directly via `SHOW data_directory;` without error suppression (no `|| true`). Strictly requires non-empty output, valid canonical path, and verifies it resides inside `/mnt/kashyap_pg`. Successful running-directory verification is mandatory before reporting `SUCCESS`.
...
* **Storage Regression Suite**: [`scripts/test-storage-verification.sh`](../../scripts/test-storage-verification.sh) expanded to 15 isolated tests verifying command failures, empty outputs, unresolvable paths, out-of-mount paths, and valid execution.
...
|---|---|:---:|---|
| **Storage Verification** | `wsl -d Ubuntu -u root -- /usr/local/bin/ensure-kashyap-pg.sh` | `0` | Verified canonical mount `/mnt/kashyap_pg` backed by `/mnt/d/Jyphra/pg_data/kashyap_pg.img`. Mandatory configured & runtime data_directory verified in D: mount. Effective log destination verified on D:. Cluster `16/kashyap` online on port 5433. |
...
| **Storage Regression Suite** | `wsl -d Ubuntu -u root -- /mnt/d/Jyphra/kashyap_family_tree/scripts/test-storage-verification.sh` | `0` | **15 passed, 0 failed**:<br>1. Non-existent image path rejected (exit 1)<br>2. Same-named image outside D: on C: rejected (exit 1)<br>3. Symlink on D: resolving outside D: rejected (exit 1)<br>4. Loop device backing mismatch rejected (exit 1)<br>5. Existing regular file on C: for logs rejected (exit 1)<br>6. Log symlink resolving outside D: rejected (exit 1)<br>7. pg_conftool failure rejected (exit 1)<br>8. pg_conftool empty output rejected (exit 1)<br>9. pg_conftool unresolvable path rejected (exit 1)<br>10. pg_conftool path outside D: mount rejected (exit 1)<br>11. SQL query failure rejected (exit 1)<br>12. SQL query empty output rejected (exit 1)<br>13. SQL query unresolvable path rejected (exit 1)<br>14. SQL query path outside D: mount rejected (exit 1)<br>15. Genuine D: storage configuration succeeds (exit 0) |
